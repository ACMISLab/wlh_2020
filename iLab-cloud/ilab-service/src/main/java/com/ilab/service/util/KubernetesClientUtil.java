...
public class KubernetesClientUtil {
    ...
    public void execEquivalent(Pod pod, String command) {
        String podName = pod.getMetadata().getName();
        String namespace = pod.getMetadata().getNamespace();
        // 1. 验证参数
        if (StringUtils.isBlank(podName) || StringUtils.isBlank(namespace)) {
        ...
        try {
            ...
            String[] execCommand = new String[] { "/bin/bash", "-c", "cd /app && pwd && " + command };
            ...
            boolean latchTerminationStatus = execLatch.await(10000, TimeUnit.SECONDS);
