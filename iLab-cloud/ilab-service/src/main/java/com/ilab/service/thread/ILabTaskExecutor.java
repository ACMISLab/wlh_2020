...
public class ILabTaskExecutor implements Runnable {
    ...
    public void run() {
        ...
        long startTime = schedulerEndTime + waitTime;
        log.info("[ILabTaskExecutor] [run] [{} 任务开始执行]", taskInfo.getTaskName());

        kubernetesClientUtil.execEquivalent(pod, taskInfo.getTaskExecScript());

        double execTime1 = TaskCostCountUtil.getTaskCost(nodeName, taskInfo.getEnvId(), TaskCostCountUtil.EXEC_TIME_ID);
        long execTime = Math.round(1000 * execTime1);
        execTime += RandomUtil.randomLong(-execTime / 100, execTime / 100);
        ...
        long endTime = startTime + execTime;
